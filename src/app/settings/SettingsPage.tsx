...
const WIDGETS = [
  { id: "now-playing",              name: "Horizontal Bar",    desc: "Sleek pill with album art",        w: 432, h: 96,  category: "normal",   supports: ["hidePaused","hideAlbumArt","radius","blur","bgOpacity","visualizer","accentColor"] },
  ...
  { id: "now-playing-notification", name: "iOS Notification",  desc: "iOS-style lock screen card",       w: 352, h: 80,  category: "normal",   supports: ["hidePaused","radius","blur","bgOpacity","accentColor"] },
  ...
  { id: "now-playing-split",        name: "Split Panel",       desc: "Art + info side by side",          w: 432, h: 152, category: "normal",   supports: ["hidePaused","radius","bgOpacity","accentColor"] },
  { id: "now-playing-vertical",     name: "Vertical",          desc: "Portrait-mode compact",            w: 242, h: 240, category: "normal",   supports: ["hidePaused","radius","bgOpacity","visualizer","accentColor"] },
  ...
  { id: "now-playing-duotone",      name: "Duotone",           desc: "Diagonal gradient accent",         w: 432, h: 110, category: "creative", supports: ["hidePaused","radius","bgOpacity","accentColor"] },
  { id: "now-playing-aura",         name: "Aura",              desc: "Full-viewport glowing background", w: 800, h: 600, category: "creative", supports: ["hidePaused","blur","bgOpacity","visualizer","accentColor"] },
  ...
  { id: "now-playing-turntable",    name: "Turntable",         desc: "Spinning vinyl record",            w: 312, h: 420, category: "creative", supports: ["hidePaused","blur","bgOpacity","accentColor"] },
  ...
  { id: "now-playing-crt",          name: "CRT TV",            desc: "Retro television set",             w: 360, h: 380, category: "creative", supports: ["hidePaused","accentColor","canvas"] },
  { id: "now-playing-canvas",       name: "Canvas",            desc: "Cinematic full-screen video",      w: 360, h: 640, category: "creative", supports: ["hidePaused","visualizer","accentColor"] },
...
export default function SettingsPage() {
  ...
  const [useBgOpacity,  setUseBgOpacity] = useState(false);
  ...
  const buildUrl = useCallback((demo = false) => {
    ...
    if (canvasMode   && has("canvas"))                    params.set("canvas",      "true");
    ...
    if (useBgOpacity && has("bgOpacity"))                 params.set("bgOpacity",   bgOpacity.toString());
    if (!visualizer  && has("visualizer"))                params.set("visualizer",  "false");
    const qs = params.toString();
    return `${base}/${selected.id}${qs ? `?${qs}` : ""}`;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selected, hidePaused, hideAlbumArt, canvasMode, useAccent, accentColor, useRadius, radius, useBlur, blur, useBgOpacity, bgOpacity, visualizer]);

  const obsUrl     = buildUrl(false);
  const previewUrl = buildUrl(true);
  ...
  return (
    <div className="min-h-screen bg-[#111] text-white" style={{ fontFamily:"system-ui,sans-serif" }}>
      ...
      <div className="flex h-[calc(100vh-65px)]">
        {/* Sidebar */}
        <aside className="shrink-0 border-r border-white/10 flex flex-col overflow-hidden transition-all duration-300"
          style={{ width: collapsed ? "0px" : "340px" }}>
          <div style={{ width:"340px" }} className="flex flex-col h-full">
            {/* Filter tabs */}
            <div className="flex gap-1 p-4 border-b border-white/10">
              ...
              <span className="ml-auto text-xs text-white/25 self-center">{visible.length + 1}</span>
            </div>
            ...
            <div className="overflow-y-auto flex-1 p-4 grid grid-cols-2 gap-2 content-start">
              ...
              {/* Custom Widget Builder — coming soon */}
              <div className="col-span-2 rounded-xl p-3 mt-1"
                style={{ background:"rgba(255,255,255,0.02)", border:"1px dashed rgba(255,255,255,0.1)" }}>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-base leading-none">🎨</span>
                  <span className="text-[11px] font-bold text-white/40">Custom Builder</span>
                  <span className="text-[8px] font-black tracking-widest px-1.5 py-0.5 rounded-full ml-auto"
                    style={{ background:"rgba(29,185,84,0.1)", color:"#1db954", border:"1px solid rgba(29,185,84,0.25)" }}>
                    SOON
                  </span>
                </div>
                <p className="text-[10px] text-white/25 leading-snug">
                  Design your own widget — layout, fonts, CSS, animations.
                </p>
              </div>
            </div>
          </div>
        ...
        {/* Main */}
        <main className="flex-1 overflow-y-auto flex gap-8 p-8 min-w-0">
          {/* Config column */}
          <section className="flex flex-col gap-5 w-[290px] shrink-0">
            ...
            <Group label="Playback">
              ...
              {has("canvas") && (
                <Row label="Spotify Canvas" sub="Show canvas video on screen (falls back to album art)">
                  <Toggle value={canvasMode} onChange={setCanvasMode} />
                </Row>
              )}
            </Group>
            ...
            <Group label="Appearance">
              {has("accentColor") && (
                <Row label="Custom accent color" sub="Override auto-extracted album color">
                  <div className="flex items-center gap-2">
                    {useAccent && (
                      <div className="relative w-8 h-8 rounded-lg overflow-hidden border border-white/20 shrink-0 cursor-pointer">
                        <input type="color" value={accentColor} onChange={e => setAccentColor(e.target.value)}
                          className="absolute inset-0 w-full h-full cursor-pointer opacity-0" />
                        <div className="absolute inset-0 rounded-lg" style={{ background: accentColor }} />
                      </div>
                    )}
                    <Toggle value={useAccent} onChange={setUseAccent} />
                  </div>
                </Row>
              )}
              ...
              {has("radius") && (
                <div className="flex flex-col gap-2">
                  <Row label="Custom corner radius" sub="Override border radius">
                    <Toggle value={useRadius} onChange={setUseRadius} />
                  </Row>
                  {useRadius && <Slider value={radius} min={0} max={24} onChange={setRadius} label="Radius" unit="px" />}
                </div>
              )}
              ...
              {has("blur") && (
                <div className="flex flex-col gap-2">
                  ...
                  {useBlur && <Slider value={blur} min={0} max={40} onChange={setBlur} label="Blur" unit="px" />}
                </div>
              )}

              {has("bgOpacity") && (
                <div className="flex flex-col gap-2">
                  <Row label="Background opacity" sub="How opaque the dark panel is">
                    <Toggle value={useBgOpacity} onChange={setUseBgOpacity} />
                  </Row>
                  {useBgOpacity && <Slider value={bgOpacity} min={0} max={100} onChange={setBgOpacity} label="Opacity" unit="%" />}
                </div>
              )}
          ...
          <section className="flex flex-col gap-4 flex-1 min-w-0">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-white/25 mt-0.5">Demo data shown — will use your real playback in OBS</p>
              </div>
